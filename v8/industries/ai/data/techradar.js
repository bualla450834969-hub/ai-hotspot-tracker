// 数据模块：techradar
window.DASHBOARD_DATA_techradar = {
  "tech_signals": {
    "collect_date": "2026-09-22",
    "total_signals": 20,
    "github_count": 12,
    "huggingface_count": 8,
    "signals": [
      {
        "name": "jev-chat/jev-chat-jarvis",
        "url": "https://github.com/jev-chat/jev-chat-jarvis",
        "description": "装在手机上的对话副驾：在微信 / QQ / X / 飞书里读懂对方、给出候选回复、一键填入输入框，发不发由你。非侵入，只读屏幕，不 hook 不改包。",
        "stars": 2199,
        "forks": 647,
        "language": "Kotlin",
        "topics": [
          "accessibility-service",
          "android",
          "chat-assistant",
          "llm",
          "qq"
        ],
        "created_at": "2026-09-21",
        "updated_at": "2026-09-22",
        "days_old": 1,
        "freshness": 100,
        "is_new": true,
        "star_growth_per_day": 2199.0,
        "heat_score": 1569.3,
        "source": "github",
        "signal_type": "开源项目",
        "analysis": {
          "tech_heat": 100,
          "social_heat": 0,
          "matched_hotwords": [],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "🔵 蓝海机会",
          "opportunity_level": "high",
          "reason": "技术热度高但社媒内容少，提前布局可抢占流量",
          "tech_terms": [
            "accessibility-service",
            "jev-chat",
            "chat-assistant",
            "jev-chat-jarvis"
          ]
        },
        "description_zh": "装在手机上的对话副驾：在微信 / QQ / X / 飞书里读懂对方、给出候选回复、一键填入输入框，发不发由你。非侵入，只读屏幕，不 hook 不改包。",
        "_gap_score": 100
      },
      {
        "name": "mizorewww/laya-mlx",
        "url": "https://github.com/mizorewww/laya-mlx",
        "description": "Native MLX runtime for Laya typed decision models — 7–14 ms short decisions on M3 Max. No text generation, PyTorch, or c",
        "stars": 4562,
        "forks": 313,
        "language": "Python",
        "topics": [
          "apple-silicon",
          "decision-model",
          "inference",
          "laya",
          "local-ai"
        ],
        "created_at": "2026-09-19",
        "updated_at": "2026-09-22",
        "days_old": 3,
        "freshness": 100,
        "is_new": true,
        "star_growth_per_day": 1520.7,
        "heat_score": 1094.5,
        "source": "github",
        "signal_type": "开源项目",
        "analysis": {
          "tech_heat": 100,
          "social_heat": 0,
          "matched_hotwords": [
            "AI",
            "AI"
          ],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "🔵 蓝海机会",
          "opportunity_level": "high",
          "reason": "技术热度高但社媒内容少，提前布局可抢占流量",
          "tech_terms": [
            "decision-model",
            "laya-mlx",
            "apple-silicon",
            "local-ai"
          ]
        },
        "description_zh": "用于Laya键入决策@ @ TERM35 @ @的本机MLX运行时— M3 Max的7–14毫秒短决策。无文本生成、PyTorch或c",
        "_gap_score": 100
      },
      {
        "name": "yibie/awesome-jev",
        "url": "https://github.com/yibie/awesome-jev",
        "description": "A curated list of public projects, integrations, and discussions built on Jev — TypeSafe AI's System One model for typed",
        "stars": 1120,
        "forks": 149,
        "language": "Python",
        "topics": [
          "awesome",
          "awesome-list",
          "jev",
          "llm"
        ],
        "created_at": "2026-09-17",
        "updated_at": "2026-09-22",
        "days_old": 5,
        "freshness": 80,
        "is_new": true,
        "star_growth_per_day": 224.0,
        "heat_score": 180.8,
        "source": "github",
        "signal_type": "开源项目",
        "analysis": {
          "tech_heat": 100,
          "social_heat": 0,
          "matched_hotwords": [
            "AI",
            "AI"
          ],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "🔵 蓝海机会",
          "opportunity_level": "high",
          "reason": "技术热度高但社媒内容少，提前布局可抢占流量",
          "tech_terms": [
            "awesome-jev",
            "awesome-list"
          ]
        },
        "description_zh": "基于JEV构建的公共项目、集成和讨论的精选列表— TypeSafe AI的System One @ @ TERM34 @ @ for typed",
        "_gap_score": 100
      },
      {
        "name": "v-modal/awesome-jev-tools",
        "url": "https://github.com/v-modal/awesome-jev-tools",
        "description": "A curated list of tools  built for Jev — TypeSafe AI's System One model for typed decisions.",
        "stars": 633,
        "forks": 14,
        "language": null,
        "topics": [
          "awesome",
          "awesome-list",
          "awesome-lists",
          "jev",
          "llm"
        ],
        "created_at": "2026-09-19",
        "updated_at": "2026-09-22",
        "days_old": 3,
        "freshness": 100,
        "is_new": true,
        "star_growth_per_day": 211.0,
        "heat_score": 177.7,
        "source": "github",
        "signal_type": "开源项目",
        "analysis": {
          "tech_heat": 100,
          "social_heat": 0,
          "matched_hotwords": [
            "AI",
            "AI"
          ],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "🔵 蓝海机会",
          "opportunity_level": "high",
          "reason": "技术热度高但社媒内容少，提前布局可抢占流量",
          "tech_terms": [
            "v-modal",
            "awesome-list",
            "awesome-jev-tools",
            "awesome-lists"
          ]
        },
        "description_zh": "为JEV打造的精选工具列表— TypeSafe AI的System One @ @ TERM34 @ @用于类型化决策。",
        "_gap_score": 100
      },
      {
        "name": "pallavi-shekhar/ai-engineering-interview-questions-company-wise",
        "url": "https://github.com/pallavi-shekhar/ai-engineering-interview-questions-company-wise",
        "description": "Your Cheat Sheet For AI Engineering Interviews at Top AI Companies - Questions and Answers.",
        "stars": 617,
        "forks": 42,
        "language": "Markdown",
        "topics": [
          "ai",
          "ai-engineering",
          "ai-engineering-interview",
          "ai-interview",
          "ai-interview-questions"
        ],
        "created_at": "2026-09-19",
        "updated_at": "2026-09-22",
        "days_old": 3,
        "freshness": 100,
        "is_new": true,
        "star_growth_per_day": 205.7,
        "heat_score": 174.0,
        "source": "github",
        "signal_type": "开源项目",
        "analysis": {
          "tech_heat": 100,
          "social_heat": 0,
          "matched_hotwords": [
            "AI",
            "AI"
          ],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "🔵 蓝海机会",
          "opportunity_level": "high",
          "reason": "技术热度高但社媒内容少，提前布局可抢占流量",
          "tech_terms": [
            "ai-interview",
            "ai-engineering-interview-questions-company-wise",
            "pallavi-shekhar",
            "ai-engineering-interview",
            "ai-engineering"
          ]
        },
        "description_zh": "您在顶级人工智能公司的人工智能工程面试备忘单-问题和答案。",
        "_gap_score": 100
      },
      {
        "name": "prism-ml/Ternary-Bonsai-2-27B-gguf",
        "url": "https://hf-mirror.com/prism-ml/Ternary-Bonsai-2-27B-gguf",
        "description": "text-generation",
        "downloads": 2227879,
        "likes": 1724,
        "tags": [
          "llama.cpp",
          "gguf",
          "ternary",
          "2-bit",
          "llama-cpp"
        ],
        "pipeline": "text-generation",
        "last_modified": "",
        "source": "huggingface",
        "signal_type": "AI模型",
        "analysis": {
          "tech_heat": 100,
          "social_heat": 0,
          "matched_hotwords": [
            "AI",
            "AI"
          ],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "🔵 蓝海机会",
          "opportunity_level": "high",
          "reason": "技术热度高但社媒内容少，提前布局可抢占流量",
          "tech_terms": [
            "llama-cpp",
            "ternary-bonsai",
            "prism-ml",
            "text-generation"
          ]
        },
        "description_zh": "Text Generation",
        "_gap_score": 100
      },
      {
        "name": "deepseek-ai/DeepSeek-V4.1-Flash",
        "url": "https://hf-mirror.com/deepseek-ai/DeepSeek-V4.1-Flash",
        "description": "image-text-to-text",
        "downloads": 512120,
        "likes": 3518,
        "tags": [
          "transformers",
          "safetensors",
          "deepseek_v41",
          "text-generation",
          "image-text-to-text"
        ],
        "pipeline": "image-text-to-text",
        "last_modified": "",
        "source": "huggingface",
        "signal_type": "AI模型",
        "analysis": {
          "tech_heat": 100,
          "social_heat": 0,
          "matched_hotwords": [
            "AI",
            "AI"
          ],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "🔵 蓝海机会",
          "opportunity_level": "high",
          "reason": "技术热度高但社媒内容少，提前布局可抢占流量",
          "tech_terms": [
            "image-text-to-text",
            "text-generation",
            "deepseek-ai"
          ]
        },
        "description_zh": "image-text-to-text",
        "_gap_score": 100
      },
      {
        "name": "Qwen/Qwen3.8-27B",
        "url": "https://hf-mirror.com/Qwen/Qwen3.8-27B",
        "description": "image-text-to-text",
        "downloads": 7153238,
        "likes": 15965,
        "tags": [
          "transformers",
          "safetensors",
          "qwen3_5",
          "image-text-to-text",
          "conversational"
        ],
        "pipeline": "image-text-to-text",
        "last_modified": "",
        "source": "huggingface",
        "signal_type": "AI模型",
        "analysis": {
          "tech_heat": 100,
          "social_heat": 0,
          "matched_hotwords": [],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "🔵 蓝海机会",
          "opportunity_level": "high",
          "reason": "技术热度高但社媒内容少，提前布局可抢占流量",
          "tech_terms": [
            "image-text-to-text"
          ]
        },
        "description_zh": "image-text-to-text",
        "_gap_score": 100
      },
      {
        "name": "wfzyx/von",
        "url": "https://github.com/wfzyx/von",
        "description": "The open-source System One decision model. Sub-15ms, non-autoregressive, local drop-in alternative to TypeSafe Jev.",
        "stars": 397,
        "forks": 30,
        "language": "Python",
        "topics": [
          "decision-model",
          "jev",
          "machine-learning",
          "python",
          "rlcd"
        ],
        "created_at": "2026-09-18",
        "updated_at": "2026-09-22",
        "days_old": 4,
        "freshness": 80,
        "is_new": true,
        "star_growth_per_day": 99.2,
        "heat_score": 93.5,
        "source": "github",
        "signal_type": "开源项目",
        "analysis": {
          "tech_heat": 82.1,
          "social_heat": 0,
          "matched_hotwords": [],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "🔵 蓝海机会",
          "opportunity_level": "high",
          "reason": "技术热度高但社媒内容少，提前布局可抢占流量",
          "tech_terms": [
            "open-source",
            "drop-in",
            "non-autoregressive",
            "decision-model",
            "machine-learning"
          ]
        },
        "description_zh": "@ @ TERM24 @ @ System One决策@ @ TERM34 @ @。Sub-15ms ，非自回归， TypeSafe Jev的局部替代品。",
        "_gap_score": 82.1
      },
      {
        "name": "devagrawal09/jev-review",
        "url": "https://github.com/devagrawal09/jev-review",
        "description": "A staged code-review workflow and local dashboard built with TypeSafe Jev.",
        "stars": 515,
        "forks": 34,
        "language": "TypeScript",
        "topics": [
          "ai",
          "code-review",
          "jev",
          "typesafe-ai",
          "typescript"
        ],
        "created_at": "2026-09-16",
        "updated_at": "2026-09-22",
        "days_old": 6,
        "freshness": 80,
        "is_new": true,
        "star_growth_per_day": 85.8,
        "heat_score": 84.1,
        "source": "github",
        "signal_type": "开源项目",
        "analysis": {
          "tech_heat": 77.6,
          "social_heat": 0,
          "matched_hotwords": [
            "AI",
            "AI"
          ],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "🔵 蓝海机会",
          "opportunity_level": "high",
          "reason": "技术热度高但社媒内容少，提前布局可抢占流量",
          "tech_terms": [
            "jev-review",
            "typesafe-ai",
            "code-review"
          ]
        },
        "description_zh": "使用TypeSafe Jev构建的分阶段代码审核@ @ TERM9 @ @和本地仪表板。",
        "_gap_score": 77.6
      },
      {
        "name": "Heman10x-NGU/openJev-verdict-2.0",
        "url": "https://github.com/Heman10x-NGU/openJev-verdict-2.0",
        "description": "Calibrated 151M Non-Autoregressive Decision Engine beating TypeSafe Jev & Laya on LocalLLaMA/typed-decisions (77.10% acc",
        "stars": 252,
        "forks": 38,
        "language": "Python",
        "topics": [
          "agentic-ai",
          "brier-score",
          "calibration",
          "decision-engine",
          "deep-learning"
        ],
        "created_at": "2026-09-19",
        "updated_at": "2026-09-22",
        "days_old": 3,
        "freshness": 100,
        "is_new": true,
        "star_growth_per_day": 84.0,
        "heat_score": 88.8,
        "source": "github",
        "signal_type": "开源项目",
        "analysis": {
          "tech_heat": 77.3,
          "social_heat": 0,
          "matched_hotwords": [
            "AI",
            "AI"
          ],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "🔵 蓝海机会",
          "opportunity_level": "high",
          "reason": "技术热度高但社媒内容少，提前布局可抢占流量",
          "tech_terms": [
            "deep-learning",
            "brier-score",
            "typed-decisions",
            "openjev-verdict",
            "non-autoregressive"
          ]
        },
        "description_zh": "校准的151M非自回归决策引擎在LocalLLaMA/类型决策上击败TypeSafe Jev和Laya （ 77.10% acc",
        "_gap_score": 77.3
      },
      {
        "name": "AbdelStark/awesome-typesafe-jev",
        "url": "https://github.com/AbdelStark/awesome-typesafe-jev",
        "description": "Awesome Jev: a source-backed field guide to TypeSafe's System One model, with SDKs, live demos, agent tools, and indepen",
        "stars": 434,
        "forks": 76,
        "language": "HTML",
        "topics": [
          "agent-skills",
          "ai-agents",
          "awesome",
          "awesome-jev",
          "awesome-list"
        ],
        "created_at": "2026-09-17",
        "updated_at": "2026-09-22",
        "days_old": 5,
        "freshness": 80,
        "is_new": true,
        "star_growth_per_day": 86.8,
        "heat_score": 84.8,
        "source": "github",
        "signal_type": "开源项目",
        "analysis": {
          "tech_heat": 77.3,
          "social_heat": 0,
          "matched_hotwords": [
            "AI",
            "AI"
          ],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "🔵 蓝海机会",
          "opportunity_level": "high",
          "reason": "技术热度高但社媒内容少，提前布局可抢占流量",
          "tech_terms": [
            "source-backed",
            "ai-agents",
            "awesome-list",
            "agent-skills",
            "awesome-typesafe-jev"
          ]
        },
        "description_zh": "Awesome Jev ： TypeSafe System One @ @ TERM34 @ @的源代码支持现场指南，包含SDK、现场演示、@ @ TERM0 @ @工具和indepen",
        "_gap_score": 77.3
      },
      {
        "name": "unstablebuild/rune",
        "url": "https://github.com/unstablebuild/rune",
        "description": "the development environment for pros",
        "stars": 1060,
        "forks": 83,
        "language": "Go",
        "topics": [
          "agent-orchestration",
          "ai",
          "cli",
          "coding-agents",
          "developer-tool"
        ],
        "created_at": "2026-09-10",
        "updated_at": "2026-09-22",
        "days_old": 12,
        "freshness": 50,
        "is_new": false,
        "star_growth_per_day": 88.3,
        "heat_score": 76.8,
        "source": "github",
        "signal_type": "开源项目",
        "analysis": {
          "tech_heat": 76.3,
          "social_heat": 0,
          "matched_hotwords": [
            "AI",
            "AI"
          ],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "🔵 蓝海机会",
          "opportunity_level": "high",
          "reason": "技术热度高但社媒内容少，提前布局可抢占流量",
          "tech_terms": [
            "agent-orchestration",
            "developer-tool",
            "coding-agents"
          ]
        },
        "description_zh": "专业人士的开发环境",
        "_gap_score": 76.3
      },
      {
        "name": "jev-chat/jev-chat-jarvis-mac",
        "url": "https://github.com/jev-chat/jev-chat-jarvis-mac",
        "description": "微信消息意图识别悬浮窗（macOS）：看屏 + 本地小模型判断意图和风险，再按话术生成回复候选。纯只读、不注入微信。",
        "stars": 86,
        "forks": 25,
        "language": "Python",
        "topics": [
          "llm",
          "local-first",
          "macos",
          "ocr",
          "privacy"
        ],
        "created_at": "2026-09-21",
        "updated_at": "2026-09-22",
        "days_old": 1,
        "freshness": 100,
        "is_new": true,
        "star_growth_per_day": 86.0,
        "heat_score": 90.2,
        "source": "github",
        "signal_type": "开源项目",
        "analysis": {
          "tech_heat": 73.5,
          "social_heat": 0,
          "matched_hotwords": [],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "🔵 蓝海机会",
          "opportunity_level": "high",
          "reason": "技术热度高但社媒内容少，提前布局可抢占流量",
          "tech_terms": [
            "local-first",
            "jev-chat-jarvis-mac",
            "jev-chat"
          ]
        },
        "description_zh": "微信消息意图识别悬浮窗（macOS）：看屏 + 本地小模型判断意图和风险，再按话术生成回复候选。纯只读、不注入微信。",
        "_gap_score": 73.5
      },
      {
        "name": "haplollc/ThinkingOrbs",
        "url": "https://github.com/haplollc/ThinkingOrbs",
        "description": "Dotted, honestly-3D loading indicators for AI and agent interfaces in SwiftUI. Nine hand-tuned designs, two tuned sizes.",
        "stars": 170,
        "forks": 14,
        "language": "Swift",
        "topics": [
          "ai",
          "animation",
          "ios",
          "loading-indicator",
          "spinner"
        ],
        "created_at": "2026-09-19",
        "updated_at": "2026-09-22",
        "days_old": 3,
        "freshness": 100,
        "is_new": true,
        "star_growth_per_day": 56.7,
        "heat_score": 69.7,
        "source": "github",
        "signal_type": "开源项目",
        "analysis": {
          "tech_heat": 64.1,
          "social_heat": 0,
          "matched_hotwords": [
            "AI",
            "AI"
          ],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "🔵 蓝海机会",
          "opportunity_level": "high",
          "reason": "技术热度高但社媒内容少，提前布局可抢占流量",
          "tech_terms": [
            "hand-tuned",
            "loading-indicator"
          ]
        },
        "description_zh": "SwiftUI中AI和@ @ TERM0 @ @接口的点状、诚实3D加载指示器。九种手工调整的设计，两种调整的尺寸。",
        "_gap_score": 64.1
      },
      {
        "name": "convaiinnovations/laya",
        "url": "https://hf-mirror.com/convaiinnovations/laya",
        "description": "text-classification",
        "downloads": 0,
        "likes": 1698,
        "tags": [
          "transformers",
          "safetensors",
          "laya",
          "system-one",
          "calibrated-decisions"
        ],
        "pipeline": "text-classification",
        "last_modified": "",
        "source": "huggingface",
        "signal_type": "AI模型",
        "analysis": {
          "tech_heat": 17.0,
          "social_heat": 0,
          "matched_hotwords": [
            "AI",
            "AI"
          ],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "👀 观察中",
          "opportunity_level": "medium",
          "reason": "热度中等，可持续观察",
          "tech_terms": [
            "system-one",
            "calibrated-decisions",
            "text-classification"
          ]
        },
        "description_zh": "文本分类"
      },
      {
        "name": "Qwen/Qwen-Image-2.1",
        "url": "https://hf-mirror.com/Qwen/Qwen-Image-2.1",
        "description": "text-to-image",
        "downloads": 6523,
        "likes": 1407,
        "tags": [
          "diffusers",
          "safetensors",
          "qwen",
          "image-generation",
          "image-editing"
        ],
        "pipeline": "text-to-image",
        "last_modified": "",
        "source": "huggingface",
        "signal_type": "AI模型",
        "analysis": {
          "tech_heat": 15.4,
          "social_heat": 0,
          "matched_hotwords": [],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "👀 观察中",
          "opportunity_level": "medium",
          "reason": "热度中等，可持续观察",
          "tech_terms": [
            "image-generation",
            "image-editing",
            "text-to-image",
            "qwen-image"
          ]
        },
        "description_zh": "text-to-image"
      },
      {
        "name": "XingChen-AGI/Xing4.0-29B-A4B",
        "url": "https://hf-mirror.com/XingChen-AGI/Xing4.0-29B-A4B",
        "description": "text-generation",
        "downloads": 18394,
        "likes": 1106,
        "tags": [
          "transformers",
          "safetensors",
          "xing4_0",
          "text-generation",
          "conversational"
        ],
        "pipeline": "text-generation",
        "last_modified": "",
        "source": "huggingface",
        "signal_type": "AI模型",
        "analysis": {
          "tech_heat": 14.7,
          "social_heat": 0,
          "matched_hotwords": [],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "👀 观察中",
          "opportunity_level": "medium",
          "reason": "热度中等，可持续观察",
          "tech_terms": [
            "xingchen-agi",
            "text-generation"
          ]
        },
        "description_zh": "Text Generation"
      },
      {
        "name": "abenzerps/Qwen-Image-2.1-GGUF",
        "url": "https://hf-mirror.com/abenzerps/Qwen-Image-2.1-GGUF",
        "description": "text-to-image",
        "downloads": 33232,
        "likes": 575,
        "tags": [
          "gguf",
          "qwen",
          "image-generation",
          "comfyui",
          "comfyui-gguf"
        ],
        "pipeline": "text-to-image",
        "last_modified": "",
        "source": "huggingface",
        "signal_type": "AI模型",
        "analysis": {
          "tech_heat": 12.4,
          "social_heat": 0,
          "matched_hotwords": [],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "👀 观察中",
          "opportunity_level": "medium",
          "reason": "热度中等，可持续观察",
          "tech_terms": [
            "image-generation",
            "text-to-image",
            "qwen-image",
            "comfyui-gguf"
          ]
        },
        "description_zh": "text-to-image"
      },
      {
        "name": "harshatheg/Qwen-2.5-1B-RLCD",
        "url": "https://hf-mirror.com/harshatheg/Qwen-2.5-1B-RLCD",
        "description": "text-generation",
        "downloads": 0,
        "likes": 513,
        "tags": [
          "mlx",
          "structured-generation",
          "parallel-decoding",
          "constrained-decoding",
          "apple-silicon"
        ],
        "pipeline": "text-generation",
        "last_modified": "",
        "source": "huggingface",
        "signal_type": "AI模型",
        "analysis": {
          "tech_heat": 5.1,
          "social_heat": 0,
          "matched_hotwords": [
            "AI",
            "AI"
          ],
          "matched_works_count": 0,
          "matched_works_avg_likes": 0,
          "opportunity": "👀 观察中",
          "opportunity_level": "medium",
          "reason": "热度中等，可持续观察",
          "tech_terms": [
            "text-generation",
            "constrained-decoding",
            "apple-silicon",
            "structured-generation",
            "parallel-decoding"
          ]
        },
        "description_zh": "Text Generation"
      }
    ],
    "forecast_topics": [
      {
        "title": "GitHub 1天2199星星，这个jev-chat-jarvis正在悄悄改变AI效率",
        "hook": "今天GitHub最值得关注的项目，3分钟教你装上并用起来",
        "signal_source": "github",
        "signal_name": "jev-chat/jev-chat-jarvis",
        "signal_url": "https://github.com/jev-chat/jev-chat-jarvis",
        "opportunity": "🔵 蓝海机会",
        "tech_heat": 100,
        "social_heat": 0,
        "category": "前瞻选题",
        "target_persona": {
          "name": "泛AI关注者",
          "traits": [
            "科技爱好者",
            "吃瓜群众",
            "投资者",
            "学生"
          ],
          "age": "16-45岁",
          "gender": "男性偏多(60%)",
          "needs": [
            "AI资讯",
            "产品测评",
            "行业趋势",
            "入门科普"
          ],
          "content_pref": "资讯速递+产品测评+趋势解读",
          "active_time": "7:30-9:00 / 12:00-13:00 / 21:00-23:00",
          "monetization": "广告(品牌合作)、私域(AI社群)",
          "pain_points": [
            "信息过载",
            "真假难辨",
            "跟不上更新",
            "不知道学啥"
          ],
          "category": "AI大类",
          "proportion": 49.3,
          "heat_score": 1675011,
          "related_keywords": [
            "AI",
            "AI",
            "AI提示词",
            "AI提示词",
            "豆包AI"
          ]
        },
        "priority": "高",
        "keyword": "jev-chat-jarvis",
        "platform": "双平台",
        "audience": "技术极客",
        "is_forecast": true
      },
      {
        "title": "刚发现的laya-mlx，AI效率效率提升10倍，知道的人还不多",
        "hook": "我测完发现最值钱的不是功能，是这个思路",
        "signal_source": "github",
        "signal_name": "mizorewww/laya-mlx",
        "signal_url": "https://github.com/mizorewww/laya-mlx",
        "opportunity": "🔵 蓝海机会",
        "tech_heat": 100,
        "social_heat": 0,
        "category": "前瞻选题",
        "target_persona": {
          "name": "泛AI关注者",
          "traits": [
            "科技爱好者",
            "吃瓜群众",
            "投资者",
            "学生"
          ],
          "age": "16-45岁",
          "gender": "男性偏多(60%)",
          "needs": [
            "AI资讯",
            "产品测评",
            "行业趋势",
            "入门科普"
          ],
          "content_pref": "资讯速递+产品测评+趋势解读",
          "active_time": "7:30-9:00 / 12:00-13:00 / 21:00-23:00",
          "monetization": "广告(品牌合作)、私域(AI社群)",
          "pain_points": [
            "信息过载",
            "真假难辨",
            "跟不上更新",
            "不知道学啥"
          ],
          "category": "AI大类",
          "proportion": 49.3,
          "heat_score": 1675011,
          "related_keywords": [
            "AI",
            "AI",
            "AI提示词",
            "AI提示词",
            "豆包AI"
          ]
        },
        "priority": "高",
        "keyword": "laya-mlx",
        "platform": "双平台",
        "audience": "技术极客",
        "is_forecast": true
      },
      {
        "title": "awesome-jev深度测评：1120星星的背后，是这个被低估的能力",
        "hook": "别人还在手动重复劳动，你已经在自动化了，这就是差距",
        "signal_source": "github",
        "signal_name": "yibie/awesome-jev",
        "signal_url": "https://github.com/yibie/awesome-jev",
        "opportunity": "🔵 蓝海机会",
        "tech_heat": 100,
        "social_heat": 0,
        "category": "前瞻选题",
        "target_persona": {
          "name": "泛AI关注者",
          "traits": [
            "科技爱好者",
            "吃瓜群众",
            "投资者",
            "学生"
          ],
          "age": "16-45岁",
          "gender": "男性偏多(60%)",
          "needs": [
            "AI资讯",
            "产品测评",
            "行业趋势",
            "入门科普"
          ],
          "content_pref": "资讯速递+产品测评+趋势解读",
          "active_time": "7:30-9:00 / 12:00-13:00 / 21:00-23:00",
          "monetization": "广告(品牌合作)、私域(AI社群)",
          "pain_points": [
            "信息过载",
            "真假难辨",
            "跟不上更新",
            "不知道学啥"
          ],
          "category": "AI大类",
          "proportion": 49.3,
          "heat_score": 1675011,
          "related_keywords": [
            "AI",
            "AI",
            "AI提示词",
            "AI提示词",
            "豆包AI"
          ]
        },
        "priority": "高",
        "keyword": "awesome-jev",
        "platform": "双平台",
        "audience": "技术极客",
        "is_forecast": true
      },
      {
        "title": "别再用笨办法了，awesome-jev-tools把AI效率自动化了",
        "hook": "免费但值钱：awesome-jev-tools的正确打开方式，看完你会回来感谢我",
        "signal_source": "github",
        "signal_name": "v-modal/awesome-jev-tools",
        "signal_url": "https://github.com/v-modal/awesome-jev-tools",
        "opportunity": "🔵 蓝海机会",
        "tech_heat": 100,
        "social_heat": 0,
        "category": "前瞻选题",
        "target_persona": {
          "name": "泛AI关注者",
          "traits": [
            "科技爱好者",
            "吃瓜群众",
            "投资者",
            "学生"
          ],
          "age": "16-45岁",
          "gender": "男性偏多(60%)",
          "needs": [
            "AI资讯",
            "产品测评",
            "行业趋势",
            "入门科普"
          ],
          "content_pref": "资讯速递+产品测评+趋势解读",
          "active_time": "7:30-9:00 / 12:00-13:00 / 21:00-23:00",
          "monetization": "广告(品牌合作)、私域(AI社群)",
          "pain_points": [
            "信息过载",
            "真假难辨",
            "跟不上更新",
            "不知道学啥"
          ],
          "category": "AI大类",
          "proportion": 49.3,
          "heat_score": 1675011,
          "related_keywords": [
            "AI",
            "AI",
            "AI提示词",
            "AI提示词",
            "豆包AI"
          ]
        },
        "priority": "高",
        "keyword": "awesome-jev-tools",
        "platform": "双平台",
        "audience": "技术极客",
        "is_forecast": true
      },
      {
        "title": "技术圈都在讨论ai-engineering-interview-questions-company-wise，但90%的人用错了方式",
        "hook": "这个项目617星星只用了3天，我整理了完整的上手教程",
        "signal_source": "github",
        "signal_name": "pallavi-shekhar/ai-engineering-interview-questions-company-wise",
        "signal_url": "https://github.com/pallavi-shekhar/ai-engineering-interview-questions-company-wise",
        "opportunity": "🔵 蓝海机会",
        "tech_heat": 100,
        "social_heat": 0,
        "category": "前瞻选题",
        "target_persona": {
          "name": "泛AI关注者",
          "traits": [
            "科技爱好者",
            "吃瓜群众",
            "投资者",
            "学生"
          ],
          "age": "16-45岁",
          "gender": "男性偏多(60%)",
          "needs": [
            "AI资讯",
            "产品测评",
            "行业趋势",
            "入门科普"
          ],
          "content_pref": "资讯速递+产品测评+趋势解读",
          "active_time": "7:30-9:00 / 12:00-13:00 / 21:00-23:00",
          "monetization": "广告(品牌合作)、私域(AI社群)",
          "pain_points": [
            "信息过载",
            "真假难辨",
            "跟不上更新",
            "不知道学啥"
          ],
          "category": "AI大类",
          "proportion": 49.3,
          "heat_score": 1675011,
          "related_keywords": [
            "AI",
            "AI",
            "AI提示词",
            "AI提示词",
            "豆包AI"
          ]
        },
        "priority": "高",
        "keyword": "ai-engineering-interview-questions-company-wise",
        "platform": "双平台",
        "audience": "技术极客",
        "is_forecast": true
      }
    ],
    "info_gap_topics": [
      {
        "title": "GitHub1天2199星星，这个jev-chat-jarvis让大模型效率翻倍，抖音还没人讲",
        "hook": "99%的人还不知道这个工具，我花了一下午踩坑，3分钟教你上手",
        "signal_source": "github",
        "signal_name": "jev-chat/jev-chat-jarvis",
        "signal_url": "https://github.com/jev-chat/jev-chat-jarvis",
        "opportunity": "🔵 蓝海机会",
        "tech_heat": 100,
        "social_heat": 0,
        "info_gap_score": 100,
        "category": "信息差选题",
        "target_persona": {
          "name": "泛AI关注者",
          "traits": [
            "科技爱好者",
            "吃瓜群众",
            "投资者",
            "学生"
          ],
          "age": "16-45岁",
          "gender": "男性偏多(60%)",
          "needs": [
            "AI资讯",
            "产品测评",
            "行业趋势",
            "入门科普"
          ],
          "content_pref": "资讯速递+产品测评+趋势解读",
          "active_time": "7:30-9:00 / 12:00-13:00 / 21:00-23:00",
          "monetization": "广告(品牌合作)、私域(AI社群)",
          "pain_points": [
            "信息过载",
            "真假难辨",
            "跟不上更新",
            "不知道学啥"
          ],
          "category": "AI大类",
          "proportion": 49.3,
          "heat_score": 1675011,
          "related_keywords": [
            "AI",
            "AI",
            "AI提示词",
            "AI提示词",
            "豆包AI"
          ]
        },
        "priority": "高",
        "keyword": "jev-chat-jarvis",
        "platform": "双平台",
        "audience": "技术极客",
        "is_forecast": true,
        "is_info_gap": true,
        "action_angle": "安装配置+第一个工作流搭建",
        "content_format": "实操教程+工作流展示",
        "monetization_hint": "完整工作流放私域，视频展示结果和片段",
        "conversion_path": {
          "private_hook": "评论区/主页引导：完整工作流+源码进群领取",
          "product_match": "AI工具工作流系列（大模型）",
          "conversion_potential": "高",
          "funnel_step": "视频展示效果→评论区钩子→私域领小样→付费转化"
        }
      },
      {
        "title": "刚冒出来的laya-mlx，AI工具的玩法全变了，现在学就是吃第一波红利",
        "hook": "技术圈已经在讨论了，抖音还没火，这就是你的机会",
        "signal_source": "github",
        "signal_name": "mizorewww/laya-mlx",
        "signal_url": "https://github.com/mizorewww/laya-mlx",
        "opportunity": "🔵 蓝海机会",
        "tech_heat": 100,
        "social_heat": 0,
        "info_gap_score": 100,
        "category": "信息差选题",
        "target_persona": {
          "name": "泛AI关注者",
          "traits": [
            "科技爱好者",
            "吃瓜群众",
            "投资者",
            "学生"
          ],
          "age": "16-45岁",
          "gender": "男性偏多(60%)",
          "needs": [
            "AI资讯",
            "产品测评",
            "行业趋势",
            "入门科普"
          ],
          "content_pref": "资讯速递+产品测评+趋势解读",
          "active_time": "7:30-9:00 / 12:00-13:00 / 21:00-23:00",
          "monetization": "广告(品牌合作)、私域(AI社群)",
          "pain_points": [
            "信息过载",
            "真假难辨",
            "跟不上更新",
            "不知道学啥"
          ],
          "category": "AI大类",
          "proportion": 49.3,
          "heat_score": 1675011,
          "related_keywords": [
            "AI",
            "AI",
            "AI提示词",
            "AI提示词",
            "豆包AI"
          ]
        },
        "priority": "高",
        "keyword": "laya-mlx",
        "platform": "双平台",
        "audience": "技术极客",
        "is_forecast": true,
        "is_info_gap": true,
        "action_angle": "接入现有AI工具链的完整方案",
        "content_format": "实操教程+工作流展示",
        "monetization_hint": "完整工作流放私域，视频展示结果和片段",
        "conversion_path": {
          "private_hook": "评论区/主页引导：完整工作流+源码进群领取",
          "product_match": "AI工具工作流系列（AI工具）",
          "conversion_potential": "高",
          "funnel_step": "视频展示效果→评论区钩子→私域领小样→付费转化"
        }
      },
      {
        "title": "awesome-jev深度拆解：技术圈已经炸了，社媒还没人讲，信息差就是钱",
        "hook": "别人还在写提示词，你已经在用awesome-jev搭工作流了",
        "signal_source": "github",
        "signal_name": "yibie/awesome-jev",
        "signal_url": "https://github.com/yibie/awesome-jev",
        "opportunity": "🔵 蓝海机会",
        "tech_heat": 100,
        "social_heat": 0,
        "info_gap_score": 100,
        "category": "信息差选题",
        "target_persona": {
          "name": "泛AI关注者",
          "traits": [
            "科技爱好者",
            "吃瓜群众",
            "投资者",
            "学生"
          ],
          "age": "16-45岁",
          "gender": "男性偏多(60%)",
          "needs": [
            "AI资讯",
            "产品测评",
            "行业趋势",
            "入门科普"
          ],
          "content_pref": "资讯速递+产品测评+趋势解读",
          "active_time": "7:30-9:00 / 12:00-13:00 / 21:00-23:00",
          "monetization": "广告(品牌合作)、私域(AI社群)",
          "pain_points": [
            "信息过载",
            "真假难辨",
            "跟不上更新",
            "不知道学啥"
          ],
          "category": "AI大类",
          "proportion": 49.3,
          "heat_score": 1675011,
          "related_keywords": [
            "AI",
            "AI",
            "AI提示词",
            "AI提示词",
            "豆包AI"
          ]
        },
        "priority": "高",
        "keyword": "awesome-jev",
        "platform": "双平台",
        "audience": "技术极客",
        "is_forecast": true,
        "is_info_gap": true,
        "action_angle": "从零到一的实操教程（避坑版）",
        "content_format": "实操教程+工作流展示",
        "monetization_hint": "完整工作流放私域，视频展示结果和片段",
        "conversion_path": {
          "private_hook": "评论区/主页引导：完整工作流+源码进群领取",
          "product_match": "AI工具工作流系列（大模型）",
          "conversion_potential": "高",
          "funnel_step": "视频展示效果→评论区钩子→私域领小样→付费转化"
        }
      },
      {
        "title": "别再手动重复操作了，awesome-jev-tools已经帮你自动化，省下来的时间都是钱",
        "hook": "这个项目3天633星星，我测完发现最值钱的不是功能，是思路",
        "signal_source": "github",
        "signal_name": "v-modal/awesome-jev-tools",
        "signal_url": "https://github.com/v-modal/awesome-jev-tools",
        "opportunity": "🔵 蓝海机会",
        "tech_heat": 100,
        "social_heat": 0,
        "info_gap_score": 100,
        "category": "信息差选题",
        "target_persona": {
          "name": "泛AI关注者",
          "traits": [
            "科技爱好者",
            "吃瓜群众",
            "投资者",
            "学生"
          ],
          "age": "16-45岁",
          "gender": "男性偏多(60%)",
          "needs": [
            "AI资讯",
            "产品测评",
            "行业趋势",
            "入门科普"
          ],
          "content_pref": "资讯速递+产品测评+趋势解读",
          "active_time": "7:30-9:00 / 12:00-13:00 / 21:00-23:00",
          "monetization": "广告(品牌合作)、私域(AI社群)",
          "pain_points": [
            "信息过载",
            "真假难辨",
            "跟不上更新",
            "不知道学啥"
          ],
          "category": "AI大类",
          "proportion": 49.3,
          "heat_score": 1675011,
          "related_keywords": [
            "AI",
            "AI",
            "AI提示词",
            "AI提示词",
            "豆包AI"
          ]
        },
        "priority": "高",
        "keyword": "awesome-jev-tools",
        "platform": "双平台",
        "audience": "技术极客",
        "is_forecast": true,
        "is_info_gap": true,
        "action_angle": "3个真实场景的应用演示",
        "content_format": "实操教程+工作流展示",
        "monetization_hint": "完整工作流放私域，视频展示结果和片段",
        "conversion_path": {
          "private_hook": "评论区/主页引导：完整工作流+源码进群领取",
          "product_match": "AI工具工作流系列（大模型）",
          "conversion_potential": "高",
          "funnel_step": "视频展示效果→评论区钩子→私域领小样→付费转化"
        }
      },
      {
        "title": "实测ai-engineering-interview-questions-company-wise：617星星的项目，最值钱的是这个工作流思路",
        "hook": "免费但值钱：ai-engineering-interview-questions-company-wise的正确打开方式，完整工作流放评论区",
        "signal_source": "github",
        "signal_name": "pallavi-shekhar/ai-engineering-interview-questions-company-wise",
        "signal_url": "https://github.com/pallavi-shekhar/ai-engineering-interview-questions-company-wise",
        "opportunity": "🔵 蓝海机会",
        "tech_heat": 100,
        "social_heat": 0,
        "info_gap_score": 100,
        "category": "信息差选题",
        "target_persona": {
          "name": "泛AI关注者",
          "traits": [
            "科技爱好者",
            "吃瓜群众",
            "投资者",
            "学生"
          ],
          "age": "16-45岁",
          "gender": "男性偏多(60%)",
          "needs": [
            "AI资讯",
            "产品测评",
            "行业趋势",
            "入门科普"
          ],
          "content_pref": "资讯速递+产品测评+趋势解读",
          "active_time": "7:30-9:00 / 12:00-13:00 / 21:00-23:00",
          "monetization": "广告(品牌合作)、私域(AI社群)",
          "pain_points": [
            "信息过载",
            "真假难辨",
            "跟不上更新",
            "不知道学啥"
          ],
          "category": "AI大类",
          "proportion": 49.3,
          "heat_score": 1675011,
          "related_keywords": [
            "AI",
            "AI",
            "AI提示词",
            "AI提示词",
            "豆包AI"
          ]
        },
        "priority": "高",
        "keyword": "ai-engineering-interview-questions-company-wise",
        "platform": "双平台",
        "audience": "技术极客",
        "is_forecast": true,
        "is_info_gap": true,
        "action_angle": "和同类工具对比，哪个更适合你",
        "content_format": "实操教程+工作流展示",
        "monetization_hint": "完整工作流放私域，视频展示结果和片段",
        "conversion_path": {
          "private_hook": "评论区/主页引导：完整工作流+源码进群领取",
          "product_match": "AI工具工作流系列（AI工具）",
          "conversion_potential": "高",
          "funnel_step": "视频展示效果→评论区钩子→私域领小样→付费转化"
        }
      }
    ],
    "summary": {
      "blue_ocean": 15,
      "exploding": 0,
      "rising": 0,
      "declining": 0,
      "watching": 5
    }
  },
  "archify_topic": {
    "title": "Archify：代码仓库秒变架构图",
    "source": "GitHub tt-a1i/archify",
    "stars": "65.9K",
    "forks": "4.5K",
    "trend": "飙升",
    "date": "2026-09",
    "summary": "Archify 是一款开源 Agent Skill，能把代码仓库或自然语言描述变成可交互的架构图。支持系统架构、工作流、调用时序、数据流向、生命周期5种图，输出自包含HTML，支持导出PNG/SVG/WebM。兼容Claude Code、Cursor、Codex CLI、OpenCode。",
    "why_hot": [
      "AI写代码越来越快，但读懂别人的代码反而更难",
      "传统Mermaid图需要手撸语法，Archify直接自然语言生成",
      "65K星说明开发者对代码可视化需求强烈",
      "Agent Skill生态正在爆发，这是标杆案例"
    ],
    "topic_suggestions": [
      "Archify实测：3分钟看懂一个陌生项目",
      "AI架构图工具横评：Archify vs Mermaid vs Excalidraw",
      "如何用Archify给老项目补文档（附教程）",
      "Archify原理拆解：为什么它能自动排版？",
      "5个Archify高级用法，让架构图直接能用"
    ],
    "audience": "全栈工程师、技术负责人、AI编程爱好者",
    "monetization": "技术教程付费、Archify定制咨询、团队培训",
    "difficulty": "中高",
    "priority": "高",
    "platform_fit": [
      "抖音",
      "B站",
      "小红书"
    ]
  }
};
