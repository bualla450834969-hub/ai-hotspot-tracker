/**
 * components/cards/hotword-card.js
 * 热词卡片组件 - 新架构版本
 * 用 DataLoader 获取数据，用 ErrorHandler 包裹
 */
(function() {
  'use strict';

  // 渲染热词表格
  function renderHotwordTable() {
    ...
    tbody.innerHTML = sorted.map((h, i) => `
      <tr>
        ...
        <td class="like-num">${h.max_like.toLocaleString()}</td>
        <td class="collect-num">${h.collect_rate}%</td>
        <td>
          <span class="tag ${trendClass(h.trend)}">${h.trend || '稳定'}</span>
        </td>
        <td>
          <span class="tag ${satMap[h.keyword] === '萌芽期' ? 'sprout' : satMap[h.keyword] === '上升期' ? 'rise' : satMap[h.keyword] === '爆发期' ? 'boom' : 'decline'}">
            ${satMap[h.keyword] || '稳定期'}
          </span>
        </td>
        <td>
          <span class="tag ${h.efficiency_tag === '蓝海' ? 'blue-ocean' : h.efficiency_tag === '红海' ? 'red-ocean' : 'medium'}">
            ${h.efficiency_tag || '适中'}
          </span>
        </td>
      </tr>
  ...
  function renderCategory() {
    ...
    charts.category.setOption({
      ...
      legend: { type: 'scroll', orient: 'vertical', right: 5, top: 'center', textStyle: { color: 'rgba(255,255,255,0.6)', fontSize: 10 } },
      series: [{
        type: 'pie', radius: ['38%', '65%'], center: ['38%', '50%'], data,
        label: { color: 'rgba(255,255,255,0.6)', fontSize: 10, formatter: '{d}%' },
        itemStyle: { borderColor: 'rgba(10,10,18,0.6)', borderWidth: 2 },
  ...
  function renderRanking() {
    ...
    charts.ranking.setOption({
      ...
      xAxis: { type: 'value', axisLabel: { color: AXIS_COLOR, formatter: v => v >= 10000 ? (v / 10000).toFixed(0) + '万' : v }, splitLine: { lineStyle: { color: SPLIT_COLOR } } },
      yAxis: { type: 'category', data: sorted.map(d => d.keyword).reverse(), axisLabel: { color: 'rgba(255,255,255,0.7)', fontSize: 11 }, axisLine: { lineStyle: { color: AXIS_LINE } } },
      series: [{
        type: 'bar', data: sorted.map(d => d.total).reverse(),
        itemStyle: { color: new echarts.graphic.LinearGradient(0, 0, 1, 0, [{ offset: 0, color: '#0A84FF' }, { offset: 1, color: '#BF5AF2' }]), borderRadius: [0, 4, 4, 0] },
        label: { show: true, position: 'right', formatter: p => p.value >= 10000 ? (p.value / 10000).toFixed(1) + '万' : p.value, fontSize: 10, color: 'rgba(255,255,255,0.6)' },
        animationDuration: 1200, animationEasing: 'cubicOut'
      }],
      tooltip: { trigger: 'axis', backgroundColor: TOOLTIP_BG, borderColor: TOOLTIP_BORDER, textStyle: { color: TOOLTIP_TEXT }, formatter: p => `${p[0].name}<br/>作品总数 ${p[0].value.toLocaleString()}` }
    });
  }
  ...
  function renderHistory() {
    ...
    const series = topKws.map((kw, i) => {
      ...
      return {
        ...
        data: vals, lineStyle: { width: 2 }, itemStyle: { color: PALETTE[i % PALETTE.length] },
        connectNulls: true,
      };
    });
  ...
  function showKeywordTrend(keyword) {
    const trends = window.DataLoader.getDashboardData().keyword_trends || {};
  ...
  function filteredHotwords() {
    const p = filterByPlatform(window.DataLoader.getHotwords());
    return currentCategory === 'all' ? p : p.filter(h => h.category === currentCategory);
  }

  // 主渲染函数 - 用 ErrorHandler 包裹
  function render() {
